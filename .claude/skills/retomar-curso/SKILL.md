...
- Caso contrário, **detecte automaticamente**:
  1. Liste os cursos disponíveis (diretórios de curso na raiz — cada um tem `CLAUDE.md` e módulos
     com `roteiro.json`; o motor fica em `engine/`, que **não** é curso):
     `ls -d */*/roteiro.json 2>/dev/null | cut -d/ -f1 | sort -u`
  2. Encontre a sessão de aula mais recente entre os cursos:
     `ls -t */.sessions/aula-*.json 2>/dev/null | head -1`
     O caminho revela o curso ativo (`<CURSO>` = 1º segmento) e o `--id` (nome do arquivo, ex.: `aula-01`).
...
O progresso (`.sessions/`) é versionado no fork/branch do aluno. Antes de carregar o estado,
...
- Se **já existe sessão**:
  - `python3 engine/aula.py status --id <ID> --curso <CURSO>`  → mapa de progresso (o que já vimos, onde estamos)
  - `python3 engine/aula.py current --id <ID> --curso <CURSO>`  → o beat atual (pontos, checkpoint, ref)
- Se **não existe sessão** para o curso escolhido:
  - Encontre o primeiro módulo (menor número): `ls -d <CURSO>/[0-9]*/ | sort | head -1`
  - Inicie: `python3 engine/aula.py start <CURSO>/NN-nome/roteiro.json`
...
`python3 engine/aula.py revisao --id <ID> --curso <CURSO>`
...
- **Em qualquer caso** (revisão feita ou dispensada), rode `python3 engine/aula.py marco --id <ID> --curso <CURSO>`
...
- **De vez em quando** (a cada poucas sessões, se já houver módulos concluídos), ofereça também a
  **revisão acumulada estilo Anki** — a skill `/revisar` (ou `python3 engine/revisar.py nova`).
...
- Quando o aluno confirmar, avance: `python3 engine/aula.py next --id <ID> --curso <CURSO>`.
- Se surgir uma dúvida importante, registre: `python3 engine/aula.py note "<texto>" --id <ID> --curso <CURSO>`.
- Em beats de fase `quiz` ou `prova`, conduza o teste com o driver de sessão:
  `python3 engine/session.py start <banco.json> [--id prova]`, apresentando as perguntas no chat,
...
2. Se houver mudanças em `<CURSO>/.sessions/`, **commite o progresso** e faça push:
   `git add <CURSO>/.sessions && git commit -m "Progresso: <curso> — <ponto onde parou>" && git push`
...
- O estado persiste em `<CURSO>/.sessions/` — por isso dá para retomar sempre de onde ficou.
