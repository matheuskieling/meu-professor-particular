...
curso com sessão mais recente (`ls -t */apps/.sessions/*.json`) ou, havendo um só curso, use-o.
Se o repo tiver remote, `git pull --ff-only` antes (ignore erro de rede, só avise).

## 2. Montar a mini-prova
Rode: `python3 <CURSO>/apps/revisar.py nova --n 8`
...
Ao terminar, rode: `python3 <CURSO>/apps/revisar.py fechar`
...
- Se o repo tiver remote e houver mudanças em `<CURSO>/apps/.sessions/`, **commite o progresso**:
  `git add <CURSO>/apps/.sessions && git commit -m "Revisão: <curso>" && git push`
...
- O histórico (`apps/.sessions/revisao-deck.json`) é versionado no fork do aluno e acompanha entre
