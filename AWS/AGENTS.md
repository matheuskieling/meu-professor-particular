...
Atalhos:
- Conduzir/retomar a aula: `python3 AWS/apps/aula.py status` + `current` (guia em `AWS/apps/CLAUDE.md`).
- Quiz/prova/simulado: `python3 AWS/apps/session.py start <questions.json>` (provas com `--id prova`,
  certificações com `--id cert`).
- Portões de prontidão para certificações: `AWS/certificacoes/CLAUDE.md`.
- Progresso do aluno: `AWS/apps/.sessions/` — commitado no fork/branch dele; `main` fica zerada.
